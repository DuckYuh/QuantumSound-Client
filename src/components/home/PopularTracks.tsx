...
export default function PopularTracks() {
    ...
    useEffect(() => {
        const fetchPopularTracks = async () => {
            try {
                const response = await trackService.getMostPopularTracks();
                            ...
                            >
                                ...
                                <div className="flex min-w-0 items-center gap-3"> 
                                    <div className="h-12 w-12 shrink-0 overflow-hidden rounded-md"> 
                                        <img src={track.album.coverImage ?? "/Logo512x512.png"} alt={track.title} className="h-full w-full object-cover" /> 
                                    </div> 
                                ...
                                <div className="flex items-center justify-end gap-3 justify-self-end" onClick={(e) => e.stopPropagation()}> 
                                    ...
                                    {user ? (
                                        <Dropdown
                                            ...
                                            placement="top"
                                            portal
                                            trigger={<Button size="sm" variant="ghost" className="p-1 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"><EllipsisVertical className="size-4" /></Button>}
                                            items={[{ label: "Add to Playlist", submenu: <PlaylistSubmenu trackId={track.id} /> }]}
                                        />
                                    ) : (
                                        <Button size="sm" variant="ghost" className="p-1 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" onClick={() => requireAuth(() => undefined)} aria-label="Open track actions">
